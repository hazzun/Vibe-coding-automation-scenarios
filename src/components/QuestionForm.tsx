...
const QuestionForm = ({ onSubmit, isLoading }: QuestionFormProps) => {
  ...
  const exampleQuestions = [
    "오케이저축은행 직원 생일 선물을 위해 예산을 추가하려고 합니다. 어떤 결재라인을 따라야 하나요?",
    "상상인저축은행 인수로 인해 사무실에 본체와 책상 등 사무용품을 새로 구매해야 합니다. 어떤 절차가 필요한가요?",
    "부서 해외 출장비로 약 3억 원 이상이 추가로 필요합니다. 어떤 결재 단계를 거쳐야 하나요?",
    "AI 활용을 위해 신규 외부 업체와 계약을 맺으려 합니다. 결재라인은 어떻게 구성되나요?"
  ];
